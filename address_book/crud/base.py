...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_pid(self, db: Session, *, pid: str) -> Optional[ModelType]:
        return db.query(self.model).filter(self.model.pid == pid).first()
    ...
    def create(self, db: Session, *, obj_in: CreateSchemaType) -> ModelType:
        ...
        db_obj.pid = helper.generate_pid("add", 25)
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
    ...
    def remove(self, db: Session, *, pid: str) -> Any:
        obj = db.query(self.model).filter(self.model.pid == pid).first()
        db.delete(obj)
        db.commit()
        return obj
